...
public class HorizontalBarChartRenderer extends BarChartRenderer {
	...
	protected void drawDataSet(Canvas c, IBarDataSet dataSet, int index) {
		...
		if (mChart.isDrawBarShadowEnabled()) {
			...
			for (int i = 0, count = Math.min(
					(int) (Math
							.ceil((float) (dataSet.getEntryCount()) * phaseX)),
				...
				if (!mViewPortHandler
						.isInBoundsTop(mBarShadowRectBuffer.bottom))
					continue;

				if (!mViewPortHandler
						.isInBoundsBottom(mBarShadowRectBuffer.top))
					break;
		...
		final boolean isCustomFill = dataSet.getFills() != null
				&& !dataSet.getFills().isEmpty();
		final boolean isSingleColor = dataSet.getColors().size() == 1;
		final boolean isInverted = mChart
				.isInverted(dataSet.getAxisDependency());
		...
		for (int j = 0, pos = 0; j < buffer.size(); j += 4, pos++) {
			...
			if (isCustomFill) {
				dataSet.getFill(pos).fillRect(c, mRenderPaint, buffer.buffer[j],
						buffer.buffer[j + 1], buffer.buffer[j + 2],
						buffer.buffer[j + 3], isInverted ? Fill.Direction.LEFT
	...
	public void drawValues(Canvas c) {
		...
		if (isDrawingValuesAllowed(mChart)) {
			...
			float posOffset = 0f;
			float negOffset = 0f;
			final boolean drawValueAboveBar = mChart
			...
			for (int i = 0; i < mChart.getBarData().getDataSetCount(); i++) {
				...
				boolean isInverted = mChart
						.isInverted(dataSet.getAxisDependency());

				// apply the text-styling defined by the DataSet
				applyValueTextStyle(dataSet);
				final float halfTextHeight = Utils.calcTextHeight(mValuePaint,
						"10") / 2f;
				...
				if (!dataSet.isStacked()) {
					...
					for (int j = 0; j < buffer.buffer.length
							* mAnimator.getPhaseX(); j += 4) {

						float y = (buffer.buffer[j + 1] + buffer.buffer[j + 3])
								/ 2f;

						if (!mViewPortHandler
								.isInBoundsTop(buffer.buffer[j + 1]))
							break;
						...
						if (!mViewPortHandler
								.isInBoundsBottom(buffer.buffer[j + 1]))
							continue;
						...
						if (dataSet.isDrawValuesEnabled()) {
							drawValue(c, formattedValue, buffer.buffer[j + 2]
					...
					while (index < dataSet.getEntryCount()
						...
						if (vals == null) {
							...
							if (!mViewPortHandler.isInBoundsTop(
									buffer.buffer[bufferIndex + 1]))
								break;

							if (!mViewPortHandler
									.isInBoundsX(buffer.buffer[bufferIndex]))
								continue;

							if (!mViewPortHandler.isInBoundsBottom(
									buffer.buffer[bufferIndex + 1]))
								continue;
							...
							for (int k = 0, idx = 0; k < transformed.length; k += 2, idx++) {
								...
								if (value == 0.0f
										&& (posY == 0.0f || negY == 0.0f)) {
									// Take care of the situation of a 0.0
									// value, which overlaps a non-zero bar
									y = value;
								} else if (value >= 0.0f) {
									posY += value;
									y = posY;
							...
							for (int k = 0; k < transformed.length; k += 2) {
								...
								final boolean drawBelow = (val == 0.0f
										&& negY == 0.0f && posY > 0.0f)
										|| val < 0.0f;
						...
						bufferIndex = vals == null ? bufferIndex + 4
								: bufferIndex + 4 * vals.length;
						index++;
					}
				}
