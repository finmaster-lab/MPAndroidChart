...
public class Legend extends ComponentBase {
	...
	/**
	 * Entries that will be appended to the end of the auto calculated entries
	 * after calculating the legend. (if the legend has already been calculated,
	 * you will need to call notifyDataSetChanged() to let the changes take
	 * effect)
	 */
	...
	/**
	 * This method sets the automatically computed colors for the legend. Use
	 * setCustom(...) to set custom colors.
	 *
	 * @param entries
	 */
	...
	/**
	 * Sets a custom legend's entries array. * A null label will start a group.
	 * This will disable the feature that automatically calculates the legend
	 * entries from the datasets. Call resetCustom() to re-enable automatic
	 * calculation (and then notifyDataSetChanged() is needed to auto-calculate
	 * the legend again)
	 */
	...
	/**
	 * @return true if a custom legend entries has been set default false
	 *         (automatic legend)
	 */
	 ...
	 * to the right/left of the chart, then this affects the width of the
	 * legend. / If the legend is to the top/bottom of the chart, then this
	 * affects the height of the legend. / default: 0.95f (95%)
	 *
	 * @param maxSize
	 */
	...
	public void calculateDimensions(Paint labelpaint,
		...
		case HORIZONTAL: {
			...
			float labelLineHeight = Utils.getLineHeight(labelpaint);
			float labelLineSpacing = Utils.getLineSpacing(labelpaint)
					+ yEntrySpace;
			...
			for (int i = 0; i < entryCount; i++) {
				...
				if (label != null) {
					...
					mCalculatedLabelSizes
							.add(Utils.calcTextSize(labelpaint, label));
					requiredWidth += drawingForm ? formToTextSpace + formSize
							: 0.f;
				...
				if (label != null || i == entryCount - 1) {
					...
					float requiredSpacing = currentLineWidth == 0.f ? 0.f
							: xEntrySpace;
					...
					if (!wordWrapEnabled // No word wrapping, it must fit.
							...
							// It simply fits
							|| (contentWidth
									- currentLineWidth >= requiredSpacing
											+ requiredWidth)) {
						// Expand current line
						...
						mCalculatedLineSizes.add(FSize.getInstance(
								currentLineWidth, labelLineHeight));
						maxLineWidth = Math.max(maxLineWidth, currentLineWidth);

						// Start a new line
						mCalculatedLabelBreakPoints.set(
								stackedStartIndex > -1 ? stackedStartIndex : i,
								true);
			...
			mNeededHeight = labelLineHeight
					* (float) (mCalculatedLineSizes.size())
					+ labelLineSpacing
							* (float) (mCalculatedLineSizes.size() == 0 ? 0
