...
public class ScatterChartRenderer extends LineScatterCandleRadarRenderer {
	...
	protected void drawDataSet(Canvas c, IScatterDataSet dataSet) {
		...
		int max = (int) (Math.min(
				Math.ceil((float) dataSet.getEntryCount()
						* mAnimator.getPhaseX()),
	...
	public void drawValues(Canvas c) {
		...
		if (isDrawingValuesAllowed(mChart)) {
			...
			List<IScatterDataSet> dataSets = mChart.getScatterData()
					.getDataSets();

			for (int i = 0; i < mChart.getScatterData()
					.getDataSetCount(); i++) {
				...
				float shapeSize = Utils
						.convertDpToPixel(dataSet.getScatterShapeSize());
				...
				for (int j = 0; j < positions.length; j += 2) {
					...
					if (dataSet.isDrawValuesEnabled()) {
						drawValue(c, formatter.getPointLabel(entry),
	...
	public void drawHighlighted(Canvas c, Highlight[] indices) {
		...
		for (Highlight high : indices) {
			...
			IScatterDataSet set = scatterData
