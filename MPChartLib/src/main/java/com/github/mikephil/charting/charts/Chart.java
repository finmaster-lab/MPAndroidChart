...
public abstract class Chart<T extends ChartData<? extends IDataSet<? extends Entry>>>
	 ...
	 */
	protected DefaultValueFormatter mDefaultValueFormatter = new DefaultValueFormatter(
			0);
	/**
	...
	/**
	 * Removes all DataSets (and thereby Entries) from the chart. Does not set
	 * the data object to null. Also refreshes the chart by calling
	 * invalidate().
	 */
	...
	protected void drawDescription(Canvas c) {
		...
		if (mDescription != null && mDescription.isEnabled()) {
			...
			if (position == null) {
				x = getWidth() - mViewPortHandler.offsetRight()
						- mDescription.getXOffset();
				y = getHeight() - mViewPortHandler.offsetBottom()
	 ...
	 * to cause it to get highlighted. Default: 500dp
	 *
	 * @param distDp
	 */
	...
	/**
	 * Set this to false to prevent values from being highlighted by tap
	 * gesture. Values can still be highlighted via drag or programmatically.
	 * Default: true
	 *
	 * @param enabled
	 */
	 ...
	 * programmatically highlight values. This method *will not* call the
	 * listener.
	 *
	 * @param highs
	 */
	...
	/**
	 * Highlights any y-value at the given x-value in the given DataSet. Provide
	 * -1 as the dataSetIndex to undo all highlighting. This method will call
	 * the listener.
	 *
	 * @param x
	 ...
	 * @param dataIndex
	 *            The data index to search in (only used in CombinedChartView
	 *            currently)
	 */
	...
	/**
	 * Highlights the value at the given x-value and y-value in the given
	 * DataSet. Provide -1 as the dataSetIndex to undo all highlighting. This
	 * method will call the listener.
	 *
	 * @param x
	 *            The x-value to highlight
	 * @param y
	 ...
	 */
	public void highlightValue(float x, float y, int dataSetIndex,
			int dataIndex, boolean callListener) {

		if (dataSetIndex < 0 || dataSetIndex >= mData.getDataSetCount()) {
			highlightValue(null, callListener);
		} else {
			highlightValue(new Highlight(x, y, dataSetIndex, dataIndex),
					callListener);
		}
	}
	 ...
	 *
	 * @param highlight
	 *            contains information about which entry should be highlighted
	 */
	 ...
	 * @param l
	 */
	public void setOnChartValueSelectedListener(
			OnChartValueSelectedListener l) {
	...
	/**
	 * Returns a recyclable MPPointF instance. Returns the center point of the
	 * chart (the whole View) in pixels.
	 *
	 * @return
	 */
	 ...
	 * @param bottom
	 */
	public void setExtraOffsets(float left, float top, float right,
			float bottom) {
	...
	/**
	 * Returns the Description object of the chart that is responsible for
	 * holding all information related to the description text that is displayed
	 * in the bottom right corner of the chart (by default).
	 *
	 * @return
	 */
	...
	/**
	 * returns true if drawing the marker is enabled when tapping on values (use
	 * the setMarker(IMarker marker) method to specify a marker)
	 *
	 * @return
	 */
	...
	/**
	 * Set this to true to draw a user specified marker when tapping on chart
	 * values (use the setMarker(IMarker marker) method to specify a marker).
	 * Default: true
	 *
	 * @param enabled
	 */
	 ...
	 * compression must be set for JPEG only. 0 == maximum compression, 100 =
	 * low compression (high quality). NOTE: Needs permission
	 ...
	 *            e.g. "Chart details"
	 * @param format
	 *            e.g. Bitmap.CompressFormat.PNG
	 * @param quality
	 *            e.g. 50, min = 0, max = 100
	 * @return returns true if saving was successful, false if not
	 */
	public boolean saveToGallery(String fileName, String subFolderPath,
			String fileDescription, Bitmap.CompressFormat format, int quality) {
		// restrain quality
		...
		File file = new File(
				extBaseDir.getAbsolutePath() + "/DCIM/" + subFolderPath);
		if (!file.exists()) {
			if (!file.mkdirs()) {
				return false;
			}
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		if (w > 0 && h > 0 && w < 10000 && h < 10000) {
			if (mLogEnabled)
				Log.i(LOG_TAG,
						"Setting chart dimens, width: " + w + ", height: " + h);
			mViewPortHandler.setChartDimens(w, h);
		} else {
			if (mLogEnabled)
				Log.w(LOG_TAG, "*Avoiding* setting chart dimens! width: " + w
						+ ", height: " + h);
		}

		// This may cause the chart view to mutate properties affecting the view
		// port --
		// lets do this before we try to run any pending jobs on the view port
		// itself
		notifyDataSetChanged();
	...
	/**
	 * Set this to true to enable "unbinding" of drawables. When a View is
	 * detached from a window. This helps avoid memory leaks. Default: false
