...
public abstract class PieRadarChartBase<T extends ChartData<? extends IDataSet<? extends Entry>>>
	...
	public void calculateOffsets() {
		...
		float legendLeft = 0f, legendRight = 0f, legendBottom = 0f,
				legendTop = 0f;
		...
		if (mLegend != null && mLegend.isEnabled()
			...
			case VERTICAL: {
				...
				if (mLegend
						.getHorizontalAlignment() == Legend.LegendHorizontalAlignment.LEFT
						|| mLegend
								.getHorizontalAlignment() == Legend.LegendHorizontalAlignment.RIGHT) {
					if (mLegend
							.getVerticalAlignment() == Legend.LegendVerticalAlignment.CENTER) {
						...
						float distReference = distanceToCenter(reference.x,
								reference.y);
			...
			case HORIZONTAL:
				...
				if (mLegend
						.getVerticalAlignment() == Legend.LegendVerticalAlignment.TOP
						|| mLegend
								.getVerticalAlignment() == Legend.LegendVerticalAlignment.BOTTOM) {

					// It's possible that we do not need this offset anymore as
					// it
					// is available through the extraOffsets, but changing it
					// can mean
		...
		float offsetBottom = Math.max(minOffset,
	...
	public void getPosition(MPPointF center, float dist, float angle,
