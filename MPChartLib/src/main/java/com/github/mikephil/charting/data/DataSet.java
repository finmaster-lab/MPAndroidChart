...
public abstract class DataSet<T extends Entry> extends BaseDataSet<T> {
	...
	/**
	 * Creates a new DataSet object with the given values (entries) it
	 * represents. Also, a label that describes the DataSet can be specified.
	 * The label can also be used to retrieve the DataSet from a ChartData
	 * object.
	 *
	 * @param entries
	 * @param label
	...
	public String toSimpleString() {
		StringBuffer buffer = new StringBuffer();
		buffer.append(
				"DataSet, label: " + (getLabel() == null ? "" : getLabel())
	...
	public void addEntryOrdered(T e) {
		...
		if (mEntries.size() > 0
				&& mEntries.get(mEntries.size() - 1).getX() > e.getX()) {
	...
	@Override
	public int getEntryIndex(float xValue, float closestToY,
			Rounding rounding) {
		...
		if (closest != -1) {
			...
			if (rounding == Rounding.UP) {
				...
				// If rounding down, and found x-value is upper than specified
				// x, and we can go lower...
			...
			if (!Float.isNaN(closestToY)) {
				while (closest > 0
						&& mEntries.get(closest - 1).getX() == closestXValue)
					closest -= 1;
