...
public class CombinedHighlighter extends ChartHighlighter<CombinedDataProvider>
	...
	public CombinedHighlighter(CombinedDataProvider chart,
		...
		barHighlighter = barChart.getBarData() == null ? null
				: new BarHighlighter(barChart);
	}
	...
	protected List<Highlight> getHighlightsAtXValue(float xVal, float x,
		...
		List<BarLineScatterCandleBubbleData> dataObjects = mChart
				.getCombinedData().getAllData();
		...
		for (int i = 0; i < dataObjects.size(); i++) {
			...
			if (barHighlighter != null && dataObject instanceof BarData) {
				...
				for (int j = 0, dataSetCount = dataObject
						.getDataSetCount(); j < dataSetCount; j++) {
