...
public class PieRadarChartTouchListener
	...
	public boolean onTouch(View v, MotionEvent event) {
		...
		// TODO: Also check if the pie itself is being touched, rather than the
		// entire chart area
		if (mChart.isRotationEnabled()) {
			...
			case MotionEvent.ACTION_MOVE:
				...
				if (mTouchMode == NONE && distance(x, mTouchStartPoint.x, y,
						mTouchStartPoint.y) > Utils.convertDpToPixel(8f)) {
	...
	private float calculateVelocity() {
		...
		// The velocity
		float velocity = Math
				.abs((lastSample.angle - firstSample.angle) / timeDelta);
