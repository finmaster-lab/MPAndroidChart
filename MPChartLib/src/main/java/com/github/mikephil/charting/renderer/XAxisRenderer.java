...
public class XAxisRenderer extends AxisRenderer {
	...
	public void computeAxis(float min, float max, boolean inverted) {
		...
		if (mViewPortHandler.contentWidth() > 10
			...
			MPPointD p1 = mTrans.getValuesByTouchPoint(
					mViewPortHandler.contentLeft(),
					mViewPortHandler.contentTop());
			MPPointD p2 = mTrans.getValuesByTouchPoint(
	...
	public void renderAxisLabels(Canvas c) {
		...
		if (mXAxis.getPosition() == XAxisPosition.TOP) {
			...
			drawLabels(c, mViewPortHandler.contentTop() + yoffset
					+ mXAxis.mLabelRotatedHeight, pointF);
		...
		for (int i = 0; i < positions.length; i += 2) {
			...
			if (mViewPortHandler.isInBoundsX(x)) {
				...
				if (mXAxis.isAvoidFirstLastClippingEnabled()) {
					...
					if (i / 2 == mXAxis.mEntryCount - 1
							&& mXAxis.mEntryCount > 1) {
	...
	protected void drawGridLine(Canvas c, float x, float y, Path gridLinePath) {
		...
		// draw a path because lines don't support dashing on lower android
		// versions
	...
	public void renderLimitLineLine(Canvas c, LimitLine limitLine,
			float[] position) {
		...
		mLimitLinePath.moveTo(mLimitLineSegmentsBuffer[0],
				mLimitLineSegmentsBuffer[1]);
		mLimitLinePath.lineTo(mLimitLineSegmentsBuffer[2],
				mLimitLineSegmentsBuffer[3]);
	...
	public void renderLimitLineLabel(Canvas c, LimitLine limitLine,
			float[] position, float yOffset) {
		...
		if (label != null && !label.equals("")) {
			...
			final LimitLine.LimitLabelPosition labelPosition = limitLine
					.getLabelPosition();

			if (labelPosition == LimitLine.LimitLabelPosition.RIGHT_TOP) {

				final float labelLineHeight = Utils
						.calcTextHeight(mLimitLinePaint, label);
