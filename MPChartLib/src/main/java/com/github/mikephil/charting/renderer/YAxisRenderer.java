...
public class YAxisRenderer extends AxisRenderer {
	 ...
	 * @param positions
	 */
	protected void drawYLabels(Canvas c, float fixedPosition, float[] positions,
			float offset) {
		...
		for (int i = from; i < to; i++) {
			...
			c.drawText(text, fixedPosition + xOffset,
	...
	public void renderGridLines(Canvas c) {
		...
		if (mYAxis.isDrawGridLinesEnabled()) {
			...
			for (int i = 0; i < positions.length; i += 2) {
				...
				// draw a path because lines don't support dashing on lower
				// android versions
	...
	/**
	 * Transforms the values contained in the axis entries to screen pixels and
	 * returns them in form of a float array of x- and y-coordinates.
	 *
	 * @return
	 */
		...
		for (int i = 0; i < limitLines.size(); i++) {
			...
			if (label != null && !label.equals("")) {
				...
				if (position == LimitLine.LimitLabelPosition.RIGHT_TOP) {
					...
					c.drawText(label, mViewPortHandler.contentRight() - xOffset,
					...
					c.drawText(label, mViewPortHandler.contentLeft() + xOffset,
							pts[1] - yOffset + labelLineHeight,
							mLimitLinePaint);
