...
public abstract class BarLineChartBase<T extends BarLineScatterCandleBubbleData<? extends IBarLineScatterCandleBubbleDataSet<? extends Entry>>>
	...
	/**
	 * the maximum number of entries to which values will be drawn (entry
	 * numbers greater than this value will cause value-labels to disappear)
	 */
	...
	/**
	 * flag indicating if the chart should stay at the same position after a
	...
	protected void init() {
		...
		mChartTouchListener = new BarLineChartTouchListener(this,
				mViewPortHandler.getMatrixTouch(), 3f);
	...
	protected void onDraw(Canvas canvas) {
		...
		if (mAxisLeft.isEnabled()
				&& mAxisLeft.isDrawLimitLinesBehindDataEnabled())
	...
	protected void prepareValuePxMatrix() {
		...
		if (mLogEnabled)
			Log.i(LOG_TAG,
					"Preparing Value-Px Matrix, xmin: " + mXAxis.mAxisMinimum
							+ ", xmax: " + mXAxis.mAxisMaximum + ", xdelta: "
							+ mXAxis.mAxisRange);

		mRightAxisTransformer.prepareMatrixValuePx(mXAxis.mAxisMinimum,
				mXAxis.mAxisRange, mAxisRight.mAxisRange,
				mAxisRight.mAxisMinimum);
		mLeftAxisTransformer.prepareMatrixValuePx(mXAxis.mAxisMinimum,
				mXAxis.mAxisRange, mAxisLeft.mAxisRange,
	...
	/**
	 * Performs auto scaling of the axis by recalculating the minimum and
	 * maximum y-values based on the entries currently in view.
	 */
	...
	public void calculateOffsets() {
		...
		if (!mCustomViewPortEnabled) {
			...
			float offsetLeft = 0f, offsetRight = 0f, offsetTop = 0f,
					offsetBottom = 0f;
			...
			if (mXAxis.isEnabled() && mXAxis.isDrawLabelsEnabled()) {
				...
				float xLabelHeight = mXAxis.mLabelRotatedHeight
						+ mXAxis.getYOffset();
	...
	/**
	 * Zooms in or out by the given scale factor. x and y are the values (NOT
	 ...
	 */
	public void zoom(float scaleX, float scaleY, float xValue, float yValue,
			AxisDependency axis) {

		Runnable job = ZoomJob.getInstance(mViewPortHandler, scaleX, scaleY,
				xValue, yValue, getTransformer(axis), axis, this);
		addViewportJob(job);
	}
	...
	/**
	 * Zooms by the specified scale factor to the specified values on the
	 * specified axis.
	 *
	 * @param scaleX
	 * @param scaleY
	 ...
	 */
	@TargetApi(11)
	public void zoomAndCenterAnimated(float scaleX, float scaleY, float xValue,
			float yValue, AxisDependency axis, long duration) {

		MPPointD origin = getValuesByTouchPoint(mViewPortHandler.contentLeft(),
				mViewPortHandler.contentTop(), axis);

		Runnable job = AnimatedZoomJob.getInstance(mViewPortHandler, this,
				getTransformer(axis), getAxis(axis), mXAxis.mAxisRange, scaleX,
				scaleY, mViewPortHandler.getScaleX(),
				mViewPortHandler.getScaleY(), xValue, yValue, (float) origin.x,
				(float) origin.y, duration);
		addViewportJob(job);
	 ...
	 * 10, no more than a range of 10 on the x-axis can be viewed at once
	 * without scrolling.
	 *
	 * @param maxXRange
	 ...
	 * 10, no less than a range of 10 on the x-axis can be viewed at once
	 * without scrolling.
	 *
	 * @param minXRange
	...
	/**
	 * Limits the maximum and minimum x range that can be visible by pinching
	 * and zooming. e.g. minRange=10, maxRange=100 the smallest range to be
	 * displayed at once is 10, and no more than a range of 100 values can be
	 * viewed at once without scrolling
	 *
	 * @param minXRange
	 * @param maxXRange
	 */
	...
	/**
	 * Sets the size of the area (range on the y-axis) that should be minimum
	 * visible at once, no further zooming in possible.
	 *
	 * @param minYRange
	 * @param axis
	 *            the axis for which this limit should apply
	...
	/**
	 * Limits the maximum and minimum y range that can be visible by pinching
	 * and zooming.
	 *
	 * @param minYRange
	 * @param maxYRange
	...
	public void moveViewTo(float xValue, float yValue, AxisDependency axis) {
		...
		Runnable job = MoveViewJob.getInstance(mViewPortHandler, xValue,
				yValue + yInView / 2f, getTransformer(axis), this);
	...
	/**
	 * This will move the left side of the current viewport to the specified
	 * x-value and center the viewport to the y value animated. This also
	 ...
	 */
	@TargetApi(11)
	public void moveViewToAnimated(float xValue, float yValue,
			AxisDependency axis, long duration) {
		...
		Runnable job = AnimatedMoveViewJob.getInstance(mViewPortHandler, xValue,
				yValue + yInView / 2f, getTransformer(axis), this,
				(float) bounds.x, (float) bounds.y, duration);
	...
	public void centerViewToY(float yValue, AxisDependency axis) {
		...
		Runnable job = MoveViewJob.getInstance(mViewPortHandler, 0f,
				yValue + valsInView / 2f, getTransformer(axis), this);
	...
	/**
	 * This will move the center of the current viewport to the specified x and
	 * y value. This also refreshes the chart by calling invalidate().
	 *
	 * @param xValue
	 * @param yValue
	 * @param axis
	 ...
	 */
	@TargetApi(11)
	public void centerViewToAnimated(float xValue, float yValue,
			AxisDependency axis, long duration) {
		...
		Runnable job = AnimatedMoveViewJob.getInstance(mViewPortHandler,
				xValue - xInView / 2f, yValue + yInView / 2f,
				getTransformer(axis), this, (float) bounds.x, (float) bounds.y,
				duration);
	...
	/**
	 * Returns a recyclable MPPointF instance. Returns the position (in pixels)
	 * the provided Entry has inside the chart view or null, if the provided
	 * Entry is null.
	 *
	 * @param e
	 * @return
	...
	public MPPointF getPosition(Entry e, AxisDependency axis) {
		...
		return MPPointF.getInstance(mGetPositionBuffer[0],
				mGetPositionBuffer[1]);
	}
	...
	/**
	 * Returns true if dragging on the X axis is enabled for the chart, false if
	 * not.
	 *
	 * @return
	 */
	...
	/**
	 * Returns true if dragging on the Y axis is enabled for the chart, false if
	 * not.
	 *
	 * @return
	 */
	...
	/**
	 * When enabled, the borders rectangle will be rendered. If this is enabled,
	 * there is no point drawing the axis-lines of x- and y-axis.
	 *
	 * @param enabled
	 */
	...
	/**
	 * When enabled, the values will be clipped to contentRect, otherwise they
	 * can bleed outside the content rect.
	 *
	 * @param enabled
	 */
	...
	/**
	 * When disabled, the data and/or highlights will not be clipped to
	 * contentRect. Disabling this option can be useful, when the data lies
	 * fully within the content rect, but is drawn in such a way (such as thick
	 * lines) that there is unwanted clipping.
	 *
	 * @param enabled
	 */
	...
	/**
	 * Returns true if keeping the position on rotation is enabled and false if
	 * not.
	 */
	...
	/**
	 * Sets whether the chart should keep its position (zoom / scroll) after a
	...
	/**
	 * Returns a recyclable MPPointD instance Returns the x and y values in the
	 * chart at the given touch point (encapsulated in a MPPointD). This method
	 * transforms pixel coordinates to coordinates / values in the chart. This
	 * is the opposite method to getPixelForValues(...).
	 *
	 * @param x
	 * @param y
	 * @return
	...
	public void getValuesByTouchPoint(float x, float y, AxisDependency axis,
			MPPointD outputPoint) {
		getTransformer(axis).getValuesByTouchPoint(x, y, outputPoint);
	}

	/**
	 * Returns a recyclable MPPointD instance Transforms the given chart values
	 * into pixels. This is the opposite method to getValuesByTouchPoint(...).
	 *
	 * @param x
	 * @param y
	 * @return
	 ...
	 * @return
	 */
	public IBarLineScatterCandleBubbleDataSet getDataSetByTouchPoint(float x,
			float y) {
		Highlight h = getHighlightByTouchPoint(x, y);
		if (h != null) {
	...
	public float getLowestVisibleX() {
		...
		float result = (float) Math.max(mXAxis.mAxisMinimum,
				posForGetLowestVisibleX.x);
		return result;
	}
	...
	public float getHighestVisibleX() {
		...
		float result = (float) Math.min(mXAxis.mAxisMaximum,
				posForGetHighestVisibleX.x);
		return result;
	}
	...
	/**
	 * If set to true, both x and y axis can be scaled simultaneously with 2
	 * fingers, if false, x and y axis can be scaled separately. default: false
	 *
	 * @param enabled
	 */
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		if (mKeepPositionOnRotation) {
			...
			getTransformer(AxisDependency.LEFT)
					.pixelsToValue(mOnSizeChangedBuffer);
		}
		...
		if (mKeepPositionOnRotation) {
			...
			getTransformer(AxisDependency.LEFT)
					.pointValuesToPixel(mOnSizeChangedBuffer);
			mViewPortHandler.centerViewPort(mOnSizeChangedBuffer, this);
		} else {
			mViewPortHandler.refresh(mViewPortHandler.getMatrixTouch(), this,
					true);
		}
	}
}
