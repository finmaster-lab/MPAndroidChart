...
public class BarHighlighter extends ChartHighlighter<BarDataProvider> {
	 ...
	 * @return
	 */
	public Highlight getStackedHighlight(Highlight high, IBarDataSet set,
		...
		if (entry.getYVals() == null) {
			...
			if (ranges.length > 0) {
				...
				Highlight stackedHigh = new Highlight(entry.getX(),
						entry.getY(), (float) pixels.x, (float) pixels.y,
						high.getDataSetIndex(), stackIndex, high.getAxis());
	...
	/**
	 * Returns the index of the closest value inside the values array / ranges
	 * (stacked barchart) to the value given as a parameter.
	 *
	 * @param ranges
	 * @param value
	 * @return
