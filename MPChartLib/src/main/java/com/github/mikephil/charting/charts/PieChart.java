...
public class PieChart extends PieRadarChartBase<PieData> {
	...
	/**
	 * Minimum angle to draw slices, this only works if there is enough room for
	 * all slices to have the minimum angle, default 0f.
	 */
	...
	public void calculateOffsets() {
		...
		// the pie-chart)
		mCircleBox.set(c.x - radius + shift, c.y - radius + shift,
	...
	protected float[] getMarkerPosition(Highlight highlight) {
		...
		float x = (float) (r * Math.cos(Math.toRadians(
				(rotationAngle + mAbsoluteAngles[entryIndex] - offset)
						* mAnimator.getPhaseY()))
				+ center.x);
		float y = (float) (r * Math.sin(Math.toRadians(
				(rotationAngle + mAbsoluteAngles[entryIndex] - offset)
						* mAnimator.getPhaseY()))
				+ center.y);
	...
	private void calcAngles() {
		...
		boolean hasMinAngle = mMinAngleForSlices != 0f
				&& entryCount * mMinAngleForSlices <= mMaxAngle;
		...
		for (int i = 0; i < mData.getDataSetCount(); i++) {
			...
			for (int j = 0; j < set.getEntryCount(); j++) {
				...
				float drawAngle = calcAngle(
						Math.abs(set.getEntryForIndex(j).getY()), yValueSum);
		...
		if (hasMinAngle) {
			// Correct bigger slices by relatively reducing their angles based
			// on the total angle needed to subtract
			// This requires that `entryCount * mMinAngleForSlices <= mMaxAngle`
			// be true to properly work!
			for (int i = 0; i < entryCount; i++) {
				minAngles[i] -= (minAngles[i] - mMinAngleForSlices) / diff
						* offset;
				if (i == 0) {
	...
	/**
	 * Enable or disable the visibility of the inner tips of the slices behind
	 * the hole
	 */
	...
	public void setCenterTextSize(float sizeDp) {
		((PieChartRenderer) mRenderer).getPaintCenterText()
				.setTextSize(Utils.convertDpToPixel(sizeDp));
	}
	...
	public void setCenterTextSizePixels(float sizePixels) {
		((PieChartRenderer) mRenderer).getPaintCenterText()
				.setTextSize(sizePixels);
	}

	/**
	 * Sets the offset the center text should have from it's original position
	 * in dp. Default x = 0, y = 0
	 *
	 * @param x
	 * @param y
	 */
	...
	/**
	 * Sets the amount of transparency the transparent circle should have 0 =
	 * fully transparent, 255 = fully opaque. Default value is 100.
	 *
	 * @param alpha
	 *            0-255
	 */
	public void setTransparentCircleAlpha(int alpha) {
		((PieChartRenderer) mRenderer).getPaintTransparentCircle()
				.setAlpha(alpha);
	}

	/**
	 * Set this to true to draw the entry labels into the pie slices (Provided
	 * by the getLabel() method of the PieEntry class). Deprecated -> use
	...
	/**
	 * Set this to true to draw the entry labels into the pie slices (Provided
	 * by the getLabel() method of the PieEntry class).
	 *
	 * @param enabled
	 */
	...
	/**
	 * Sets whether to draw slices in a curved fashion, only works if drawing
	 * the hole is enabled and if the slices are not drawn under the hole.
	 *
	 * @param enabled
	 *            draw curved ends of slices
	 */
	...
	/**
	 * Sets the max angle that is used for calculating the pie-circle. 360f
	 * means it's a full PieChart, 180f results in a half-pie-chart. Default:
	 * 360f
	 *
	 * @param maxangle
	 *            min 90, max 360
	...
	/**
	 * Set the angle to set minimum size for slices, you must call
	 * {@link #notifyDataSetChanged()} and {@link #invalidate()} when changing
	 * this, only works if there is enough room for all slices to have the
	 * minimum angle.
	 *
	 * @param minAngle
	 *            minimum 0, maximum is half of {@link #setMaxAngle(float)}
	 */
