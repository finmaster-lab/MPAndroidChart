...
public class BarChart extends BarLineChartBase<BarData>
	 ...
	 * Performance-intensive code should use void getBarBounds(BarEntry, RectF)
	 * instead.
	 *
	 * @param e
	 * @return
	 ...
	 * the specified Entry in the specified DataSet. The rect will be assigned
	 * Float.MIN_VALUE in all locations if the Entry could not be found in the
	 * charts data.
	 *
	 * @param e
	 * @return
	...
	public void getBarBounds(BarEntry e, RectF outputRect) {
		...
		if (set == null) {
			bounds.set(Float.MIN_VALUE, Float.MIN_VALUE, Float.MIN_VALUE,
					Float.MIN_VALUE);
			return;
		}
	 ...
	 * to make it highlight single values (relevant only for stacked). If
	 * enabled, highlighting operations will highlight the whole bar, even if
	 * only a single stack entry was tapped. Default: false
	 *
	 * @param enabled
	 */
	 ...
	 * allow the bars of the barchart to be fully displayed. Default: false
	 *
	 * @param enabled
	 */
	...
	/**
	 * Groups all BarDataSet objects this data object holds together by
	 * modifying the x-value of their entries. Previously set x-values of
	 * entries will be overwritten. Leaves space between bars and groups as
	 * specified by the parameters. Calls notifyDataSetChanged() afterwards.
	 *
	 * @param fromX
	 *            the starting point on the x-axis where the grouping should
	 *            begin
	 * @param groupSpace
	 *            the space between groups of bars in values (not pixels) e.g.
	 *            0.8f for bar width 1f
	 * @param barSpace
	 *            the space between individual bars in values (not pixels) e.g.
	 *            0.1f for bar width 1f
	 */
