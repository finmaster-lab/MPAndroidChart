...
public class RadarChartRenderer extends LineRadarRenderer {
	...
	protected void drawDataSet(Canvas c, IRadarDataSet dataSet,
		...
		if (dataSet.getEntryCount() > mostEntries) {
			// if this is not the largest set, draw a line to the center before
			// closing
		...
		if (dataSet.isDrawFilledEnabled()) {
			...
			if (drawable != null) {
				...
				drawFilledPath(c, surface, dataSet.getFillColor(),
		...
		for (int i = 0; i < mChart.getData().getDataSetCount(); i++) {
			...
			for (int j = 0; j < dataSet.getEntryCount(); j++) {
				...
				if (dataSet.isDrawValuesEnabled()) {
					drawValue(c, formatter.getRadarLabel(entry), pOut.x,
				...
				if (entry.getIcon() != null && dataSet.isDrawIconsEnabled()) {
					...
					Utils.drawImage(c, icon, (int) pIcon.x, (int) pIcon.y,
	...
	protected void drawWeb(Canvas c) {
		...
		int maxEntryCount = mChart.getData().getMaxEntryCountSet()
				.getEntryCount();
		...
		for (int j = 0; j < labelCount; j++) {
			...
			for (int i = 0; i < mChart.getData().getEntryCount(); i++) {
				...
				float r = (mChart.getYAxis().mEntries[j]
						- mChart.getYChartMin()) * factor;

				Utils.getPosition(center, r, sliceangle * i + rotationangle,
						p1out);
				Utils.getPosition(center, r,
						sliceangle * (i + 1) + rotationangle, p2out);
			...
			if (set.isDrawHighlightCircleEnabled()) {
				...
				if (!Float.isNaN(pOut.x) && !Float.isNaN(pOut.y)) {
					...
					if (set.getHighlightCircleStrokeAlpha() < 255) {
						strokeColor = ColorTemplate.colorWithAlpha(strokeColor,
								set.getHighlightCircleStrokeAlpha());
					}

					drawHighlightCircle(c, pOut,
							set.getHighlightCircleInnerRadius(),
							set.getHighlightCircleOuterRadius(),
							set.getHighlightCircleFillColor(), strokeColor,
	...
	public void drawHighlightCircle(Canvas c, MPPointF point, float innerRadius,
			float outerRadius, int fillColor, int strokeColor,
			float strokeWidth) {
		c.save();
		...
		if (strokeColor != ColorTemplate.COLOR_NONE) {
			...
			mHighlightCirclePaint
					.setStrokeWidth(Utils.convertDpToPixel(strokeWidth));
