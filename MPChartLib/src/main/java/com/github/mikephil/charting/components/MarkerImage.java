...
public class MarkerImage implements IMarker {
	 ...
	 *            the drawable resource to render
	 */
	...
	public MPPointF getOffsetForDrawingAtPoint(float posX, float posY) {
		...
		if (posX + mOffset2.x < 0) {
			mOffset2.x = -posX;
				...
				&& posX + width + mOffset2.x > chart.getWidth()) {
		...
		if (posY + mOffset2.y < 0) {
			mOffset2.y = -posY;
				...
				&& posY + height + mOffset2.y > chart.getHeight()) {
	...
	public void draw(Canvas canvas, float posX, float posY) {
		...
		mDrawable.setBounds(mDrawableBoundsCache.left, mDrawableBoundsCache.top,
