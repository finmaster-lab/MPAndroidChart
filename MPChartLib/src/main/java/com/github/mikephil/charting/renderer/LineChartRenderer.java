...
public class LineChartRenderer extends LineRadarRenderer {
	...
	protected void drawHorizontalBezier(ILineDataSet dataSet) {
		...
		if (mXBounds.range >= 1) {
			...
			for (int j = mXBounds.min + 1; j <= mXBounds.range
					+ mXBounds.min; j++) {
				...
				cubicPath.cubicTo(cpx, prev.getY() * phaseY, cpx,
						cur.getY() * phaseY, cur.getX(), cur.getY() * phaseY);
			}
		}
	...
	protected void drawCubicBezier(ILineDataSet dataSet) {
		...
		if (mXBounds.range >= 1) {
			...
			// That's because we need 4 points for a cubic bezier (cubic=4),
			// otherwise we get lines moving and doing weird stuff on the edges
			// of the chart.
			...
			for (int j = mXBounds.min + 1; j <= mXBounds.range
				...
				cubicPath.cubicTo(prev.getX() + prevDx,
						(prev.getY() + prevDy) * phaseY, cur.getX() - curDx,
						(cur.getY() - curDy) * phaseY, cur.getX(),
						cur.getY() * phaseY);
			}
		}
	...
	protected void drawCubicFill(Canvas c, ILineDataSet dataSet, Path spline,
			Transformer trans, XBounds bounds) {

		float fillMin = dataSet.getFillFormatter().getFillLinePosition(dataSet,
				mChart);

		spline.lineTo(
				dataSet.getEntryForIndex(bounds.min + bounds.range).getX(),
				fillMin);
		...
		if (drawable != null) {
			...
			drawFilledPath(c, spline, dataSet.getFillColor(),
	...
	protected void drawLinear(Canvas c, ILineDataSet dataSet) {
		...
		final boolean isDrawSteppedEnabled = dataSet
				.getMode() == LineDataSet.Mode.STEPPED;
		...
		if (dataSet.getColors().size() > 1) {
			...
			for (int j = mXBounds.min; j < max; j++) {
				...
				// Determine the start and end coordinates of the line, and make
				// sure they differ.
				...
				if (!mViewPortHandler.isInBoundsLeft(lastCoordinateX)
						|| !mViewPortHandler.isInBoundsTop(
								Math.max(firstCoordinateY, lastCoordinateY))
						|| !mViewPortHandler.isInBoundsBottom(
								Math.min(firstCoordinateY, lastCoordinateY)))
					continue;

				// get the color that is set for this line-segment
				mRenderPaint.setColor(dataSet.getColor(j));

				canvas.drawLines(mLineBuffer, 0, pointsPerEntryPair * 2,
						mRenderPaint);
			}

		} else { // only one color per dataset

			if (mLineBuffer.length < Math.max((entryCount) * pointsPerEntryPair,
					pointsPerEntryPair) * 2)
				mLineBuffer = new float[Math.max(
			...
			if (e1 != null) {
				...
				int j = 0;
				for (int x = mXBounds.min; x <= mXBounds.range
						+ mXBounds.min; x++) {
				...
				if (j > 0) {
					...
					final int size = Math.max(
							(mXBounds.range + 1) * pointsPerEntryPair,
							pointsPerEntryPair) * 2;
	...
	protected void drawLinearFill(Canvas c, ILineDataSet dataSet,
		...
		// Doing this iteratively in order to avoid OutOfMemory errors that can
		// happen on large bounds sets.
		do {
			currentStartIndex = startingIndex + (iterations * indexInterval);
			currentEndIndex = currentStartIndex + indexInterval;
			currentEndIndex = currentEndIndex > endingIndex ? endingIndex
					: currentEndIndex;

			if (currentStartIndex <= currentEndIndex) {
				generateFilledPath(dataSet, currentStartIndex, currentEndIndex,
						filled);
				...
				if (drawable != null) {
					...
					drawFilledPath(c, filled, dataSet.getFillColor(),
							dataSet.getFillAlpha());
				}
			}
	 ...
	 *
	 * @param dataSet
	 *            The dataset from which to read the entries.
	 * @param startIndex
	 *            The index from which to start reading the dataset
	 * @param endIndex
	 *            The index from which to stop reading the dataset
	 * @param outputPath
	 *            The path object that will be assigned the chart data.
	 * @return
	 */
	private void generateFilledPath(final ILineDataSet dataSet,
			final int startIndex, final int endIndex, final Path outputPath) {

		final float fillMin = dataSet.getFillFormatter()
		...
		if (isDrawingValuesAllowed(mChart)) {
			...
			for (int i = 0; i < dataSets.size(); i++) {
				...
				for (int j = 0; j < positions.length; j += 2) {
					...
					Entry entry = dataSet
							.getEntryForIndex(j / 2 + mXBounds.min);

					if (dataSet.isDrawValuesEnabled()) {
						drawValue(c, formatter.getPointLabel(entry), x,
								y - valOffset,
	...
	protected void drawCircles(Canvas c) {
		...
		for (int i = 0; i < dataSets.size(); i++) {
			...
			if (changeRequired) {
				imageCache.fill(dataSet, drawCircleHole,
	...
	public void drawHighlighted(Canvas c, Highlight[] indices) {
		...
		for (Highlight high : indices) {
			...
			ILineDataSet set = lineData
					.getDataSetByIndex(high.getDataSetIndex());
	...
	/**
	 * Sets the Bitmap.Config to be used by this renderer. Default:
	 * Bitmap.Config.ARGB_8888 Use Bitmap.Config.ARGB_4444 to consume less
	 * memory.
	 *
	 * @param config
	 */
		...
		protected void fill(ILineDataSet set, boolean drawCircleHole,
			...
			for (int i = 0; i < colorCount; i++) {
				...
				Bitmap circleBitmap = Bitmap.createBitmap(
						(int) (circleRadius * 2.1), (int) (circleRadius * 2.1),
						conf);
				...
				if (drawTransparentCircleHole) {
					...
					// Cut hole in path
					mCirclePathBuffer.addCircle(circleRadius, circleRadius,
							circleHoleRadius, Path.Direction.CCW);
