...
public class YAxisRendererRadarChart extends YAxisRenderer {
	...
	public YAxisRendererRadarChart(ViewPortHandler viewPortHandler, YAxis yAxis,
			RadarChart chart) {
	...
	protected void computeAxisValues(float min, float max) {
		...
		// This is used to avoid repeated values when rounding values for
		// display.
		if (mAxis.isGranularityEnabled())
			interval = interval < mAxis.getGranularity()
					? mAxis.getGranularity()
					: interval;

		// Normalize interval
		double intervalMagnitude = Utils.roundToNextSignificant(
				Math.pow(10, (int) Math.log10(interval)));
		int intervalSigDigit = (int) (interval / intervalMagnitude);
		if (intervalSigDigit > 5) {
			// Use one order of magnitude higher, to avoid intervals like 0.9 or
			// 90
			// if it's 0.0 after floor(), we use the old value
			interval = Math.floor(10.0 * intervalMagnitude) == 0.0 ? interval
		...
		if (mAxis.isForceLabelsEnabled()) {
			...
			double first = interval == 0.0 ? 0.0
					: Math.ceil(yMin / interval) * interval;
			if (centeringEnabled) {
				first -= interval;
			}

			double last = interval == 0.0 ? 0.0
					: Utils.nextUp(Math.floor(yMax / interval) * interval);
			...
			for (f = first, i = 0; i < n; f += interval, ++i) {
				...
				if (f == 0.0) // Fix for negative zero case (Where value ==
								// -0.0, and 0.0 == -0.0)
					f = 0.0;
	...
	public void renderAxisLabels(Canvas c) {
		...
		final int to = mYAxis.isDrawTopYLabelEntryEnabled() ? mYAxis.mEntryCount
	...
	public void renderLimitLines(Canvas c) {
		...
		for (int i = 0; i < limitLines.size(); i++) {
			...
			for (int j = 0; j < mChart.getData().getMaxEntryCountSet()
					.getEntryCount(); j++) {
