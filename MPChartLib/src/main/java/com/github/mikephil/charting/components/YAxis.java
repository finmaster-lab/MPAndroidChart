...
public class YAxis extends AxisBase {
	...
	/**
	 * flag that indicates if the zero-line should be drawn regardless of other
	 * grid lines
	 */
	...
	/**
	 * the maximum width that the axis can take (in dp). use Inifinity for
	 * disabling the maximum default: Float.POSITIVE_INFINITY (no maximum
	 * specified)
	 */
	...
	/**
	 * This method is deprecated. Use setAxisMinimum(...) / setAxisMaximum(...)
	 * instead.
	 *
	 * @param startAtZero
	 */
	...
	public float getRequiredWidthSpace(Paint p) {
		...
		width = Math.max(minWidth,
				Math.min(width, maxWidth > 0.0 ? maxWidth : width));
	...
	public void calculate(float dataMin, float dataMax) {
		...
		// Discussion:
		// https://github.com/danielgindi/Charts/pull/3650#discussion_r221409991
		if (min > max) {
		...
		// calc extra spacing
		this.mAxisMinimum = mCustomAxisMin ? this.mAxisMinimum
				: min - (range / 100f) * getSpaceBottom();
		this.mAxisMaximum = mCustomAxisMax ? this.mAxisMaximum
				: max + (range / 100f) * getSpaceTop();
