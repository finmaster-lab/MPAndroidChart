 ...
 *
 *
 * Cost/Benefit : Cost - The pool can only contain objects extending Poolable.
 * Benefit - The pool can very quickly determine if an object is elligable for
 * storage without iteration. Benefit - The pool can also know if an instance of
 * Poolable is already stored in a different pool instance. Benefit - The pool
 * can grow as needed, if it is empty Cost - However, refilling the pool when it
 * is empty might incur a time cost with sufficiently large capacity. Set the
 * replenishPercentage to a lower number if this is a concern.
 *
...
public class ObjectPool<T extends ObjectPool.Poolable> {
	...
	/**
	 * Returns an ObjectPool instance, of a given starting capacity, that
	 * recycles instances of a given Poolable object.
	 *
	 * @param withCapacity
	 ...
	 * @return
	 */
	public static synchronized ObjectPool create(int withCapacity,
			Poolable object) {
	...
	/**
	 * Set the percentage of the pool to replenish on empty. Valid values are
	 * between 0.00f and 1.00f
	 *
	 * @param percentage
	 *            a value between 0 and 1, representing the percentage of the
	 *            pool to replenish.
	 */
	 ...
	 * The T instance passed must not already exist inside this or any other
	 * ObjectPool instance.
	 *
	 * @param object
	...
	/**
	 * Recycle a List of Poolables that this pool is capable of generating. The
	 * T instances passed must not already exist inside this or any other
	 * ObjectPool instance.
	 *
	 * @param objects
	 *            A list of objects of type T to recycle
	 */
	public synchronized void recycle(List<T> objects) {
		while (objects.size() + this.objectsPointer
				+ 1 > this.desiredCapacity) {
			this.resizePool();
		}
	...
	/**
	 * Returns the capacity of this object pool. Note : The pool will
	 * automatically resize to contain additional objects if the user tries to
	 * add more objects than the pool's capacity allows, but this comes at a
	 * performance cost.
	 *
	...
	/**
	 * Returns the number of objects remaining in the pool, for diagnostic
	 * purposes.
	 *
