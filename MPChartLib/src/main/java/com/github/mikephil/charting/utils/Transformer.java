...
public class Transformer {
	...
	public float[] generateTransformedValuesBubble(IBubbleDataSet data,
		...
		final int count = (to - from + 1) * 2; // (int) Math.ceil((to - from) *
												// phaseX) * 2;
	...
	public float[] generateTransformedValuesLine(ILineDataSet data,
			float phaseX, float phaseY, int min, int max) {
	...
	/**
	 * Returns a recyclable MPPointD instance. Returns the x and y coordinates
	 * (pixels) for a given x and y value in the chart.
	 *
	 * @param x
	 * @param y
	 * @return
