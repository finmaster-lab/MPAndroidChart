...
public interface IDataSet<T extends Entry> {
	...
	/**
	 * Returns the number of y-values this DataSet represents -> the size of the
	 * y-values array -> yvals.size()
	 *
	 * @return
	 */
	int getEntryCount();
	...
	/**
	 * Calculates the minimum and maximum x and y values (mXMin, mXMax, mYMin,
	 * mYMax).
	 */
	void calcMinMax();

	/**
	 * Calculates the min and max y-values from the Entry closest to the given
	 * fromX to the Entry closest to the given toX value. This is only needed
	 * for the autoScaleMinMax feature.
	 *
	 * @param fromX
	 * @param toX
	 */
	 ...
	 * @param rounding
	 *            determine whether to round up/down/closest if there is no
	 *            Entry matching the provided x-value
	 * @return
	 *
	 *
	 */
	 ...
	 * Returns the first Entry object found at the given x-value with binary
	 * search. If the no Entry at the specified x-value is found, this method
	 * returns the Entry at the closest x-value. INFORMATION: This method does
	 ...
	 * An empty array if no Entry object at that x-value. INFORMATION: This
	...
	/**
	 * Returns the Entry object found at the given index (NOT xIndex) in the
	 * values array.
	 *
	 * @param index
	 * @return
	 ...
	 * @return
	 */
	int getEntryIndex(float xValue, float closestToY,
			DataSet.Rounding rounding);
	...
	/**
	 * This method returns the actual index in the Entry array of the DataSet
	 * for a given xIndex. IMPORTANT: This method does calculations at runtime,
	...
	/**
	 * Adds an Entry to the DataSet dynamically. Entries are added to the end of
	 * the list. This will also recalculate the current minimum and maximum
	...
	/**
	 * Adds an Entry to the DataSet dynamically. Entries are added to their
	 * appropriate index in the values array respective to their x-position.
	 * This will also recalculate the current minimum and maximum values of the
	 * DataSet and the value-sum.
	 *
	 * @param e
	 */
	...
	/**
	 * Removes the first Entry (at index 0) of this DataSet from the entries
	 * array. Returns true if successful, false if not.
	 *
	 * @return
	 */
	boolean removeFirst();
	...
	/**
	 * Removes the last Entry (at index size-1) of this DataSet from the entries
	 * array. Returns true if successful, false if not.
	 *
	 * @return
	 */
	...
	/**
	 * Removes the Entry object at the given index in the values array from the
	 * DataSet. Returns true if an Entry was removed, false if no Entry could be
	 * removed.
	 *
	 * @param index
	 * @return
	...
	/**
	 * Removes all values from this DataSet and does all necessary
	 * recalculations.
	 */
	void clear();
	 ...
	 * contains. This is only used for performance reasons when only one color
	 * is in the colors array (size == 1)
	 *
	 * @return
	 */
	int getColor();
	...
	/**
	 * Returns only the first color of all colors that are set to be used for
	 * the values.
	 *
	 * @return
	 */
	...
	/**
	 * Returns the color at the specified index that is used for drawing the
	 * values inside the chart. Uses modulus internally.
	 *
	 * @param index
	 * @return
	 */
	...
	/**
	 * Returns the text size that is used for drawing the values inside the
	 * chart
	 *
	 * @return
	 */
	 ...
	 *
	 * NOTE (for bar and line charts): if `maxVisibleCount` is reached, no
	 ...
	 *
	 * NOTE (for bar and line charts): if `maxVisibleCount` is reached, no icons
	 * will be drawn even if this is enabled
	 *
	 * @param enabled
	 */
	 ...
	 *
	 * For all charts except Pie and Radar it will be ordinary (x offset,y
	 * offset).
	 *
	 * For Pie and Radar chart it will be (y offset, distance from center
	 * offset); so if you want icon to be rendered under value, you should
	 * increase X component of CGPoint, and if you want icon to be rendered
	 * closet to center, you should decrease height component of CGPoint.
	 *
	 * @param offset
	 */
