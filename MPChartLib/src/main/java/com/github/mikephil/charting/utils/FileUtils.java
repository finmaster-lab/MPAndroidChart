...
public class FileUtils {
			...
			while ((line = br.readLine()) != null) {
				...
				if (split.length <= 2) {
					...
					entries.add(new BarEntry(
							Integer.parseInt(split[split.length - 1]), vals));
				}
			}
		} catch (IOException e) {
	 ...
	 * @return
	 */
	public static List<Entry> loadEntriesFromAssets(AssetManager am,
			String path) {
		...
		try {
			...
			while (line != null) {
				...
				if (split.length <= 2) {
					entries.add(new Entry(Float.parseFloat(split[1]),
	...
	public static List<BarEntry> loadBarEntriesFromAssets(AssetManager am,
		...
		try {
			...
			while (line != null) {
				...
				entries.add(new BarEntry(Float.parseFloat(split[1]),
						Float.parseFloat(split[0])));
