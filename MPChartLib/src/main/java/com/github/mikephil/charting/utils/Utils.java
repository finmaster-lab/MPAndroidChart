...
public abstract class Utils {
	...
	/**
	 * Returns a recyclable FSize instance. calculates the approximate size of a
	 * text, depending on a demo text avoid repeated calls (e.g. inside drawing
	 * methods)
	 *
	 * @param paint
	 * @param demoText
	...
	/**
	 * calculates the approximate size of a text, depending on a demo text avoid
	 * repeated calls (e.g. inside drawing methods)
	 *
	 * @param paint
	 * @param demoText
	 * @param outputFSize
	 *            An output variable, modified by the function.
	 */
	public static void calcTextSize(Paint paint, String demoText,
			FSize outputFSize) {
	...
	/// - returns: The default value formatter used for all chart components
	 ...
	 * number as a string, maximum 35 characters. If thousands are separated,
	 * the separating character is a dot (".").
	 *
	 * @param number
	 * @param digitCount
	 * @param separateThousands
	 ...
	 */
	public static String formatNumber(float number, int digitCount,
			boolean separateThousands, char separateChar) {
		...
		while (lval != 0 || charCount < (digitCount + 1)) {
			...
			} else if (separateThousands && lval != 0
	...
	public static float roundToNextSignificant(double number) {
		...
		final float d = (float) Math
				.ceil((float) Math.log10(number < 0 ? -number : number));
	...
	/**
	 * Returns a recyclable MPPointF instance. Calculates the position around a
	 * center point, depending on the distance from the center, and the angle of
	 * the position around the center.
	 *
	 * @param center
	 * @param dist
	 * @param angle
	 ...
	 * @return
	 */
	public static MPPointF getPosition(MPPointF center, float dist,
			float angle) {
	...
	public static void getPosition(MPPointF center, float dist, float angle,
			MPPointF outputPoint) {
		outputPoint.x = (float) (center.x
				+ dist * Math.cos(Math.toRadians(angle)));
		outputPoint.y = (float) (center.y
				+ dist * Math.sin(Math.toRadians(angle)));
	}
	...
	public static void drawImage(Canvas canvas, Drawable drawable, int x, int y,
		...
		drawable.setBounds(mDrawableBoundsCache.left, mDrawableBoundsCache.top,
	...
	public static void drawXAxisValue(Canvas c, String text, float x, float y,
			Paint paint, MPPointF anchor, float angleDegrees) {
		...
		if (angleDegrees != 0.f) {
			...
			// Move the text drawing rect in a way that it always rotates around
			// its center
			...
			// Move the "outer" rect relative to the anchor, assuming its
			// centered
	...
	public static void drawMultilineText(Canvas c, StaticLayout textLayout,
			float x, float y, TextPaint paint, MPPointF anchor,
			float angleDegrees) {
		...
		if (angleDegrees != 0.f) {
			...
			if (anchor.x != 0.5f || anchor.y != 0.5f) {
				final FSize rotatedSize = getSizeOfRotatedRectangleByDegrees(
						drawWidth, drawHeight, angleDegrees);
	...
	public static void drawMultilineText(Canvas c, String text, float x,
			float y, TextPaint paint, FSize constrainedToSize, MPPointF anchor,
			float angleDegrees) {

		StaticLayout textLayout = new StaticLayout(text, 0, text.length(),
				paint, (int) Math.max(Math.ceil(constrainedToSize.width), 1.f),
	...
	/**
	 * Returns a recyclable FSize instance. Represents size of a rotated
	 * rectangle by degrees.
	 *
	 * @param rectangleSize
	 * @param degrees
	 * @return A Recyclable FSize instance
	 */
	public static FSize getSizeOfRotatedRectangleByDegrees(FSize rectangleSize,
			float degrees) {
		final float radians = degrees * FDEG2RAD;
		return getSizeOfRotatedRectangleByRadians(rectangleSize.width,
				rectangleSize.height, radians);
	}

	/**
	 * Returns a recyclable FSize instance. Represents size of a rotated
	 * rectangle by radians.
	 *
	 * @param rectangleSize
	 * @param radians
	 * @return A Recyclable FSize instance
	 */
	public static FSize getSizeOfRotatedRectangleByRadians(FSize rectangleSize,
			float radians) {
		return getSizeOfRotatedRectangleByRadians(rectangleSize.width,
				rectangleSize.height, radians);
	}
	...
	public static FSize getSizeOfRotatedRectangleByDegrees(float rectangleWidth,
		...
		final float radians = degrees * FDEG2RAD;
		return getSizeOfRotatedRectangleByRadians(rectangleWidth,
				rectangleHeight, radians);
	}
	...
	public static FSize getSizeOfRotatedRectangleByRadians(float rectangleWidth,
		...
		return FSize.getInstance(
				Math.abs(rectangleWidth * (float) Math.cos(radians))
						+ Math.abs(rectangleHeight * (float) Math.sin(radians)),
				Math.abs(rectangleWidth * (float) Math.sin(radians)) + Math
						.abs(rectangleHeight * (float) Math.cos(radians)));
	}
